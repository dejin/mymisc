...
static int berlin_ion_print_info(struct berlin_ion_info *info)
{
	...
	printk("\033[31mMarvell Berlin ION information dejin\033[00m\n");
...
static int berlin_ion_sync(struct ion_berlin_data *region)
{
	...
	if (region->len == 0) {
		printk("%s invalid parameter gid[%d] virtaddr[%x] len[%d]\n",
			__func__, region->gid, region->addr, region->len);
		dump_stack();
		return -EINVAL;
	}
...
static int berlin_udp_init(void)
{
	...
	netlink_set_nonroot(NETLINK_BERLIN_CC,
...
nlmsg_failure:
	printk("%s nlmsg_failure (SID = 0x%08X, size = %d )\n",
			__func__, dstserviceID, msglen);
	kfree_skb(skb);
	return -1;
}
...
{
	int ret;
	printk("\033[31m%s be call\033[00m\n", __func__);
	dump_stack();
	if (msgbuf == NULL) {
